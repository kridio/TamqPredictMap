...
public class Gradient {
    ...
    private HashMap<Integer, ColorInterval> generateColorIntervals() {
        ...
        if(this.mStartPoints[0] != 0.0F) {
            ...
            colorIntervals.put(Integer.valueOf(0), new ColorInterval(i, this.mColors[0], (float)this.mColorMapSize * this.mStartPoints[0]));
        }

        for(i = 1; i < this.mColors.length; ++i) {
            colorIntervals.put(Integer.valueOf((int)((float)this.mColorMapSize * this.mStartPoints[i - 1])), new ColorInterval(this.mColors[i - 1], this.mColors[i], (float)this.mColorMapSize * (this.mStartPoints[i] - this.mStartPoints[i - 1])));
        }

        if(this.mStartPoints[this.mStartPoints.length - 1] != 1.0F) {
            i = this.mStartPoints.length - 1;
            colorIntervals.put(Integer.valueOf((int)((float)this.mColorMapSize * this.mStartPoints[i])), new ColorInterval(this.mColors[i], this.mColors[i], (float)this.mColorMapSize * (1.0F - this.mStartPoints[i])));
        }
